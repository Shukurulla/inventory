...
import { Search, X, MapPin, Calendar, User } from "lucide-react";
...
export const Header: React.FC<HeaderProps> = ({ title, children }) => {
  ...
  const parseSpecifications = (specString: string | null) => {
    ...
    try {
      return typeof specString === "string"
  ...
  const renderSpecifications = (equipment: Tequipment) => {
    ...
    if (equipment.computer_details || equipment.computer_specification_data) {
      const computerSpecs =
        parseSpecifications(equipment.computer_details) ||
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-2 text-sm">
              ...
              {computerSpecs.storage && (
                <div>
                  <span className="font-medium">Накопитель:</span>{" "}
                  {computerSpecs.storage}
                </div>
              )}
              {computerSpecs.monitor_size && (
                <div>
                  <span className="font-medium">Монитор:</span>{" "}
                  {computerSpecs.monitor_size}"
                </div>
              )}
              <div>
                <span className="font-medium">Клавиатура:</span>{" "}
                {computerSpecs.has_keyboard ? "Есть" : "Нет"}
              </div>
              <div>
                <span className="font-medium">Мышь:</span>{" "}
                {computerSpecs.has_mouse ? "Есть" : "Нет"}
              </div>
            </div>
          </div>
        );
    ...
    if (equipment.projector_char || equipment.projector_specification_data) {
      const projectorSpecs =
        parseSpecifications(equipment.projector_char) ||
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-2 text-sm">
              ...
              {projectorSpecs.throw_type && (
                <div>
                  <span className="font-medium">Тип проекции:</span>{" "}
                  {projectorSpecs.throw_type}
                </div>
              )}
            </div>
    ...
    if (equipment.printer_char || equipment.printer_specification_data) {
      const printerSpecs =
        parseSpecifications(equipment.printer_char) ||
        ...
        specs.push(
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-2 text-sm">
              ...
              <div>
                <span className="font-medium">Тип печати:</span>{" "}
                {printerSpecs.color ? "Цветной" : "Чёрно-белый"}
              </div>
              <div>
                <span className="font-medium">Duplex:</span>{" "}
                {printerSpecs.duplex ? "Есть" : "Нет"}
              </div>
            </div>
          </div>
        );
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-3">
                ...
                {renderSpecifications(selectedEquipment).length > 0 ? (
                  <div className="space-y-3">
                    {renderSpecifications(selectedEquipment)}
                  </div>
                ) : (
