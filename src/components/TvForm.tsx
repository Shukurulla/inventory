...
import { useCreateTvSpecsMutation, useGetTvSpecsQuery} from "@/api/universityApi";
...
export const TvForm: React.FC<createFormPropsType> = ({ equipmentFormData, setEquipmentFormData, create, onOpenChange }) => {
    ...
    const handleInputChange = (field: keyof TVSpecs, value: string | boolean | number) => {
                    ...
                    <Select onValueChange={handleTemplateChange}>
                        ...
                        <SelectContent>
                            {specs?.map((item, index) => (
                                <SelectItem key={item.id} value={item?.id?.toString() || index.toString()}>{item.model+ ' '  + item.screen_size+'dyum'}</SelectItem>
                            ))}
                        </SelectContent>
                    </Select>
                </div>
