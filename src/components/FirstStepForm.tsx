...
export const StepOneForm = ({ data, setData }: { data: createEquipmentBodyType; setData: React.Dispatch<React.SetStateAction<createEquipmentBodyType>> }) => {
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="flex items-center gap-2 justify-between relative">
                <p className="text-lg flex-1 text-accent-foreground">Фото техники:
                    {data.photo && (
                        <span className="text-sm text-gray-500">{(data.photo as File).name}</span>
                    )}
                </p>
                <Input
                    type="file"
                    ...
                    <label htmlFor="file-upload" className="cursor-pointer">
                        <Button variant="secondary" className="gap-1 bg-indigo-600 text-accent-foreground hover:bg-indigo-400 text-white text-lg h-12">
            ...
            <div className="flex items-center gap-2">
                <Label htmlFor="template" className="text-black/40">Договор:</Label>
                <Select onValueChange={(value) => setData({ ...data, contract_id: Number(value) })}>
                    <SelectTrigger className="w-full !h-12 border-gray-300 focus:ring-indigo-600 focus:border-indigo-600">
                        <SelectValue placeholder="Выберите договор"/>
                    </SelectTrigger>
                    <SelectContent>
                        {isLoading ? <Skeleton className="w-full border-b h-7"/> : contracts?.results?.map((item) => (
                            <SelectItem key={item.id} value={item.id.toString()}>{item.number + ' - ' + item.valid_until}</SelectItem>
                        ))}
                    </SelectContent>
                </Select>
            </div>
