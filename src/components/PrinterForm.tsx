...
import { useCreatePrinterSpecsMutation, useGetPrinterSpecsQuery } from "@/api/universityApi";
...
export const PrinterForm:React.FC<createFormPropsType> = ({equipmentFormData, setEquipmentFormData, create, onOpenChange }) => {
      ...
      <form>
         <div className="grid grid-cols-2 gap-4">
            <div className="flex flex-col space-y-2 text-md">
               <Label htmlFor="model" className="text-black/40 dark:text-accent-foreground">
                  Модель:
               </Label>
               <Input
                  id="model"
            ...
            <div className="flex items-center flex-1 col-span-1 pt-5 text-lg">
                <div className="flex items-center space-x-2 flex-1">
                    <Checkbox
                        id="color"
                        className="w-6 h-6 border-indigo-600 text-indigo-600 data-[state=checked]:bg-indigo-600 data-[state=checked]:text-white"
                        checked={!!formData.color}
                        onCheckedChange={(checked) => handleInputChange("color", checked as boolean)}
                    />
                    <Label htmlFor="color" className="text-accent-foreground">Цветной/Чёрно-белый</Label>
                </div>
                <div className="flex items-center space-x-2 flex-1">
                    <Checkbox
                        id="duplex"
                        ...
                        onCheckedChange={(checked) => handleInputChange("duplex", checked as boolean)}
                    />
                    <Label htmlFor="duplex" className="text-accent-foreground">Двусторонняя/Односторонняя печать</Label>
                </div>
            </div>
         </div>
         {!create && <div className="flex col-span-1 pt-3 gap-3">
            <div className="flex-1 flex flex-col space-y-2">
               <Label htmlFor="template" className="text-black/40 dark:text-accent-foreground">
                  ...
                  <SelectContent>
                     {specs?.map((item) => (
                        <SelectItem key={item.id} value={item?.id?.toString() || ""}>
                           {item.model + " " + (item.color ? "Цветной" : "Чёрно-белый")}
                        </SelectItem>
                     ))}
                  </SelectContent>
               </Select>
            ...
            <div className="flex-1 flex flex-col space-y-2">
               <Label htmlFor="quantity" className="text-black/40 dark:text-accent-foreground">
                  Количество:
               </Label>
               <Input
                  id="quantity"
