import { useDeleteEquipmentsMutation, useGetAddedEquipmentsQuery, useGetBlocksQuery, useGetEquipmentsQuery, useGetEquipmentTypesQuery, useGetRoomsQuery } from "@/api/universityApi";
...
const { data: equipments = { results: [] }, refetch } = useGetEquipmentsQuery();

  const [deleteEquipment] = useDeleteEquipmentsMutation();

const totalItems = equipments?.results?.length || 0;
  const totalPages = Math.ceil(totalItems / itemsPerPage);
  const {data: allAddets = []} = useGetAddedEquipmentsQuery()
  const filterByType = () => {
  ...
  return (
		<>
			<div className="flex space-x-4 mb-6">
				<Select onValueChange={(value) => setSelectedBlock(Number(value) || null)} value={selectedBlock?.toString() || ""}>
				...
				<Select onValueChange={(value) => setSelectedRoom(Number(value) || null)} value={selectedRoom?.toString() || ""}>
					<SelectTrigger className="w-40 bg-white border-gray-300 dark:border-zinc-800 rounded-lg">
				...
				<Select onValueChange={(value) => setSelectedEquipmentType(Number(value) || null)} value={selectedEquipmentType?.toString() || ""}>
			...
			<div className="rounded-xl overflow-hidden border dark:border-zinc-800">
				<div className="p-0">
					{addetsPaginate.map((equipment: {id: number, name: string, data: Tequipment[]}) => (
						<CustomAccordion
							key={equipment.id}
							...
							triggerContent={
								<>
									<IconLabel
										icon={() => (
											<div className={`w-12 h-12 rounded-full flex items-center justify-center ${
												inventoryIcons.find((icon) => icon.name === equipment.name)?.color || "bg-blue-50 dark:bg-accent"
											}`}
											>
												<div className="text-gray-600">
													{inventoryIcons.find((icon) => icon.name === equipment.name)?.icon || <CircleIcon />}
												</div>
											</div>
										)}
										color="#000"
									...
									<div className="flex-1">
										<div className="bg-indigo-50 dark:bg-zinc-900 dark:text-indigo-400 w-12 h-12 rounded-full flex items-center justify-center">{equipment.data.length}</div>
									</div>
								</>
							}
						>
						{equipment.data.map(item => (
							...
							>
								<div className="flex items-center space-x-3">
									<div className={`w-12 h-12 rounded-full ${inventoryIcons.find((icon) => icon.name === equipment.name)?.color} flex items-center justify-center`}>
										{inventoryIcons.find((icon) => icon.name === equipment.name)?.icon || <CircleIcon />}
									</div>
									<span className="text-accent-foreground text-xl font-medium">{item.name}</span>
								</div>
